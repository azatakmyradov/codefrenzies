...
const updatePost = () => {
    form.put(route('dashboard.posts.update', { id: props.post.id }), {
        preserveScroll: true,
        onStart: () => {
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-6">
                        <form action="#" @submit.prevent="updatePost">
                            <AdminInput name="title"
                                        :errors="form.errors.title"
                                        v-model="form.title"
                                        :default-value="form.title"/>

                            <AdminInput name="slug"
                                        :errors="form.errors.slug"
                                        v-model="form.slug"
                                        :default-value="form.slug" />

                            <AdminInput name="seo_description"
                                        :errors="form.errors.seo_description"
                                        v-model="form.seo_description"
                                        :default-value="form.seo_description" />

                            <AdminInput name="thumbnail"
                                        :errors="form.errors.thumbnail"
                                        v-model="form.thumbnail"
                                        :default-value="form.thumbnail"/>
                            ...
                            <MarkdownEditor :errors="form.errors.body" v-model="form.body" :default-value="form.body" />
